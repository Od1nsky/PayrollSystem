<template>
  <div>
    <div class="d-flex justify-space-between align-center mb-6">
      <h1 class="text-h4">Начисления</h1>
    ...
    <v-card>
      <v-data-table
        ...
        :loading="loading"
        no-data-text="Начисления не найдены"
    ...
    <v-dialog v-model="showDeleteDialog" max-width="400">
      <v-card>
        ...
        <v-card-text>
          Вы уверены, что хотите удалить это начисление?
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showDeleteDialog = false">Отмена</v-btn>
          <v-btn color="error" :loading="deleting" @click="deleteAccrual">Удалить</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
const accrualToDelete = ref<any>(null)
...
const accrualTypes = [
  ...
  { title: 'Премия', value: 'bonus' },
  { title: 'Отпускные', value: 'vacation' },
  { title: 'Больничный', value: 'sick_leave' },
  { title: 'Сверхурочные', value: 'overtime' },
]
...
const submitForm = async () => {
  ...
  try {
    await api.post('/accruals', formData.value)
    toast.success('Начисление успешно создано')
    closeModal()
    await fetchAccruals()
  } catch (e: any) {
...
const confirmDelete = (accrual: any) => {
  accrualToDelete.value = accrual
  showDeleteDialog.value = true
}

const deleteAccrual = async () => {
  if (!accrualToDelete.value) return

  deleting.value = true
  try {
    await api.delete(`/accruals/${accrualToDelete.value.id}`)
    toast.success('Начисление успешно удалено')
    showDeleteDialog.value = false
    accrualToDelete.value = null
    await fetchAccruals()
  } catch (e: any) {
    const message = getReadableErrorMessage(e)
    toast.error(message)
...
const fetchAccruals = async () => {
  loading.value = true
  try {
    const response = await api.get('/accruals')
    const result = response.data as any
    accruals.value = result.data || result || []
  } catch (error: any) {
