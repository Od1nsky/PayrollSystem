import { Router } from 'express';
import { body } from 'express-validator';
import { DeductionController } from '../controllers/deductionController';
import { authenticate } from '../middleware/auth';
import { validate } from '../middleware/validation';
import { DeductionType } from '../types';

const router = Router();
router.use(authenticate);

const createValidation = [
  body('employeeId').isInt({ min: 1 }).withMessage('Сотрудник обязателен'),
  body('type').isIn(Object.values(DeductionType)).withMessage('Некорректный тип удержания'),
  body('amount').isFloat({ min: 0 }).withMessage('Сумма должна быть положительной'),
  body('period').matches(/^\d{4}-\d{2}$/).withMessage('Период в формате YYYY-MM'),
  body('date').isISO8601().withMessage('Некорректная дата'),
  validate,
];

router.get('/', DeductionController.getAll);
router.get('/:id', DeductionController.getById);
router.post('/', createValidation, DeductionController.create);
router.put('/:id', DeductionController.update);
router.delete('/:id', DeductionController.delete);

export default router;
